...
class PlgK2Png2K2 extends K2Plugin
{
	var $pluginName = 'png2k2';
	var $pluginNameHumanReadable = 'PNG / GIF images for K2 items';
	var $_new = null;
	var $_old = null;
	...
	private function _isAnimatedGif($filename)
	{
		return (bool)preg_match('#(\x00\x21\xF9\x04.{4}\x00\x2C.*){2,}#s', file_get_contents($filename));
	}

	private function _scaleImageFile($fileSrc, $w, $h, $saveTo, $resizemethod = 1)
	{
		$delays = array(5);
		...
		if(file_exists($fileSrc) && is_numeric($w))
		{
			if(list($width, $height, $type, $attr) = getimagesize($fileSrc))
			{
				...
				{
					if(!($oldimg = $this->_loadImage($fileSrc, $type)))
					{
						return false;
					}
	...
	private function _loadImage($fileSrc, $imgType)
	{
		switch($imgType)
		{
			case 1:   //   gif
	...
	private function _outputImage($img, $saveTo)
	{
		if(strlen($saveTo) > 0)
		{
